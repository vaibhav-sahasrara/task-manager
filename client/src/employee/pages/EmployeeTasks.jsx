...
const EmployeeTasks = () => {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-indigo-700 dark:text-yellow-300 mb-8 flex items-center gap-3">
        Tasks Assigned
      </h1>

      {tasks.length === 0 ? (
        <div className="text-center text-gray-400 dark:text-gray-500 mt-20 text-lg animate-pulse flex flex-col items-center gap-2">
          <FiLoader className="text-3xl" />
          No tasks assigned yet.
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {tasks.map((task, index) => (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: index * 0.05 }}
              className="relative p-5 bg-white dark:bg-gray-900 bg-opacity-80 dark:bg-opacity-90 backdrop-blur-md border border-white/20 dark:border-gray-700 shadow-2xl rounded-3xl overflow-hidden group hover:scale-[1.02] transition-transform duration-300"
            >
              {/* Gradient top bar */}
              <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500" />

              <div className="space-y-3">
                <h2 className="text-xl font-semibold text-indigo-800 dark:text-white flex items-center gap-2">
                  {task.name}
                  <span
                    className={`text-xs px-2 py-1 rounded-full font-bold ${getPriorityColor(
                      task.priority
                ...
                <div className="space-y-2 text-sm pt-2 text-gray-700 dark:text-gray-300">
                  <div className="flex items-center gap-2">
                    <FiCalendar className="text-indigo-400" />
                    Deadline:{" "}
                    <span className="font-medium">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <select
                      ...
                      className="ml-2 bg-white dark:bg-gray-800 text-gray-800 dark:text-white px-3 py-1 text-sm rounded-full border border-gray-300 dark:border-gray-600 shadow-sm hover:bg-white dark:hover:bg-gray-700 transition"
                    >
                      <option>To Do</option>
                      <option>In Progress</option>
                      <option>Done</option>
