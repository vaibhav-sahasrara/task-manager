...
const TeamModal = ({
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editMode) {
        await axios.put(`/api/team/${newMember._id}`, payload, {
          headers: { Authorization: `Bearer ${token}` },
        });
      } else {
        await axios.post("/api/team", payload, {
      ...
      {showModal && (
        ...
        >
          <motion.div
            ...
            exit={{ scale: 0.9 }}
            transition={{ type: "spring", bounce: 0.3 }}
            className="bg-white dark:bg-gray-900 dark:text-gray-100 rounded-2xl shadow-xl w-full max-w-lg p-6 relative"
          >
            {/* Close Button */}
            <button
              className="absolute top-2 right-3 text-gray-400 hover:text-red-500 text-2xl"
            ...
            <div className="grid grid-cols-2 gap-4">
              {["name", "email", "role", "phone"].map((field) => (
                <input
                  ...
                  type="text"
                  placeholder={field.charAt(0).toUpperCase() + field.slice(1)}
                  className="px-4 py-2 border rounded-md bg-white dark:bg-gray-800 dark:border-gray-700 dark:text-white"
                  value={newMember[field]}
                  onChange={(e) =>
                    setNewMember({ ...newMember, [field]: e.target.value })
                  }
              ...
              <input
                type="text"
                placeholder="Avatar URL"
                className="col-span-2 px-4 py-2 border rounded-md bg-white dark:bg-gray-800 dark:border-gray-700 dark:text-white"
                value={newMember.avatar}
                onChange={(e) =>
                  setNewMember({ ...newMember, avatar: e.target.value })
                }
