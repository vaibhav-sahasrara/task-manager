...
const Projects = () => {
  ...
  return (
    <div className="space-y-8 text-gray-900 dark:text-gray-100">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold text-gray-600 dark:text-white">Projects</h1>
        <button
          onClick={() => setShowAddForm(true)}
          className="px-5 py-2 flex items-center gap-2 rounded-full bg-gradient-to-r from-indigo-500 to-purple-500 text-white font-medium shadow-lg hover:opacity-90"
        >
          <FiPlus /> New Project
        </button>
      </div>
      ...
      {loading ? (
        <div className="text-center text-gray-400 dark:text-gray-300">Loading projects...</div>
      ) : projects.length === 0 ? (
        <div className="text-center text-gray-400 dark:text-gray-300">No projects found.</div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
          {projects.map((project) => (
            <ProjectCard
